...
<% 
	...
	try{
		...
		result.next();
   		...
   		out.print(result.getString("date").substring(5,7));
   		//Process the result
   		while (result.next()) { 
   			map=new HashMap<String,Double>();
   			int i = 0;
   			String currentMonth = "";
   			if(result.getString("date").substring(5,7).equals("01")){
   				currentMonth = "January";
   			}
   			if(result.getString("date").substring(5,7).equals("02")){
   				currentMonth = "February";
   			}
   			if(result.getString("date").substring(5,7).equals("03")){
   				currentMonth = "March";
   			}
   			if(result.getString("date").substring(5,7).equals("04")){
   				currentMonth = "April";
   			}
   			if(result.getString("date").substring(5,7).equals("05")){
   				currentMonth = "May";
   			}
   			if(result.getString("date").substring(5,7).equals("06")){
   				currentMonth = "June";
   			}
   			if(result.getString("date").substring(5,7).equals("07")){
   				currentMonth = "July";
   			}
   			if(result.getString("date").substring(5,7).equals("08")){
   				currentMonth = "August";
   			}
   			if(result.getString("date").substring(5,7).equals("09")){
   				currentMonth = "September";
   			}
   			if(result.getString("date").substring(5,7).equals("10")){
   				currentMonth = "October";
   			}
   			if(result.getString("date").substring(5,7).equals("11")){
   				currentMonth = "November";
   			}
   			if(result.getString("date").substring(5,7).equals("12")){
   				currentMonth = "December";
   			}
   	   		for(i = 0; i<list.size(); i++){
   				if(((list.get(i)).keySet().toArray()[0]).equals(currentMonth)){
   					double newVal = result.getDouble("total_price") + (list.get(i)).get(currentMonth);
   					map.put(currentMonth, newVal);
   					list.set(i, map);
   				}
   			}
